...
class Feature:
    def __init__(self, form):
        ...
        self.precision = (99)
        prot = ""
        self.feature_data_path = '/home/goetz/feature-3.1.0/data'
        ...
        To run Feature, you have to install wsl2 (Win10 buid 2004 or higher).<br> Install Ubunto (for wsl) from the Windows store
        and install gcc and compile FEATURE 3.1 found at : https://simtk.org/projects/feature in the linux wsl environment.<br>
        ...
        def get_feature_data_path(self):
            dirname = '/home/'+os.path.normcase(os.environ.get('USERNAME'))+'/feature-3.1.0/data'
            # This is a workaround as code below using UNC path names doesn't work in QFileDialog
            # dirdialog = QtWidgets.QFileDialog()
            # ubuntu_path = os.path.normpath('//wsl$/Ubuntu-16.04')
            # print(ubuntu_path)
            # dirname = dirdialog.getExistingDirectory(None, "feature data location", ubuntu_path)
        ...
        def read_plugin_config_file():
            config_file_name = os.path.join(tmp_dir,"new2_feature_plugin.conf")
            self.config_settings = {}
        ...
        def run_feature(): 
            ...
            if ( not os.path.isfile(gridfile)):
                ...
                if ( not os.path.isfile(model)):
                    ...
                    current_path="/mnt/c"+os.path.abspath(os.curdir).split(":")[-1]
                    current_posix_path=posixer(current_path)
                    self.pdb_path = "/mnt/c"+self.pdb_dir_path.split(":")[-1]
                    ...
                    command = 'call wsl bash -li < %s' % (filename)
                    print(command)
                    os.system(command)
